...
def render_panel(res, title_prefix, date_desc):
    ...
    # 組合三欄式卡片 HTML (使用字串拼接，完全不使用開頭多餘縮排)
    panel_html = (
        f'<div class="combined-box">'
        f'<div class="combined-col combined-core">'
        f'<div class="section-subcaption">{title_prefix} ‧ 核心指標</div>'
        f'<div class="core-metrics-wrapper">'
        f'<div class="metric-item"><span class="metric-label">🎯 目標數</span><span class="metric-value-gold">{res["goal_num"]}</span></div>'
        f'<div class="metric-item"><span class="metric-label">🔢 格局數</span><span class="metric-value-gold">{res["pattern_num"]}</span></div>'
        f'<div class="metric-item"><span class="metric-label">☯️ 命格屬性</span><div style="display:flex; align-items:center;"><span class="metric-value-text">{pattern_main}</span>{badge_html}</div></div>'
        f'</div>'
        f'</div>'
        f'<div class="combined-col combined-matrix">'
        f'<div class="section-subcaption">{title_prefix} ‧ 神煞排盤矩陣</div>'
        f'<div class="matrix-wrapper">'
        f'<div class="matrix-row">{top_boxes_html}</div>'
        f'<hr class="matrix-divider">'
        f'<div class="matrix-row">{bottom_boxes_html}</div>'
        f'</div>'
        f'</div>'
        f'<div class="combined-col combined-energy">'
        f'<div class="section-subcaption">{title_prefix} ‧ 格局能量排列</div>'
        f'<div class="energy-wrapper">{layout_content}</div>'
        f'</div>'
        f'</div>'
    )
    ...
    fieldset_html = (
        f'<fieldset class="tk-fieldset">'
        f'<legend class="tk-legend">{title_prefix} - 詳細計算過程</legend>'
        f'<div class="tk-text-area">{detail_text}</div>'
        f'</fieldset>'
    )

    st.markdown(fieldset_html, unsafe_allow_html=True)
